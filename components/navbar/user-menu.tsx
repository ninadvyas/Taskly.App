    ...
    return (
        <Popover>
            <PopoverTrigger asChild>
                <button className="rounded-sm outline-none ring-offset-background focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                    <Avatar className="h-8 w-8 cursor-pointer rounded-sm">
                        {user.image && <AvatarImage src={user.image} alt={user.name ?? ""} />}
                        <AvatarFallback className="text-xs rounded-sm">{initials}</AvatarFallback>
                    </Avatar>
                </button>
            </PopoverTrigger>
